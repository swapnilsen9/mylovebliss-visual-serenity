...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'glass-effect shadow-lg' : 'bg-brand-primarywhite'
    }`}>
      <div className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex space-x-8">
            <button onClick={() => scrollToTop()} className={`font-larken ${
              isScrolled ? 'text-brand-primarywhite drop-shadow-sm' : 'text-brand-secondaryBlack'
          ...
          <button 
            className={`md:hidden ${
              isScrolled ? 'text-brand-primarywhite' : 'text-brand-secondaryBlack'
            }`}
